...
$rm = $_GET['rm'];
if ($rm == null) {}
else {if ($link->query("DELETE FROM support WHERE id=$rm") === TRUE) {$msg = '<div class="alert alert-success text-center" id="alert"><i class="bi bi-check-circle"></i> Das Ticket #'.$rm.' wurde gelöscht!</div>';}}
    ...
    <div class="container-md">
        <div class="row align-items-start mt-4">
            ...
            <div class="col"><table class="w-100 table table-striped">
                ...
                <tbody>
                <?php 
                $result = mysqli_query($link, "SELECT * FROM support WHERE closed=1");
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo('<tr>
                            ...
                            <td>'.$row["time"].'</td>
                            <td>
                                <a href="?rm='.$row["id"].'" style="color: red; padding: 3px;" title="Ticket Löschen"><i class="bi bi-trash3-fill"></i></a>
                            </td></tr>');
                    }
                } else {echo "<h1>Keine Tickets geschlossen.</h1>";}
                ?>
                </tbody>
            </table></div>
        </div>
...
<script>
    $("#alert").delay(4500).queue(function() {$(this).remove(); window.location.href = "./closed";});
</script>
<script>
